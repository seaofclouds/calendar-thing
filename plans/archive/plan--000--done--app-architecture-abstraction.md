# App Architecture Abstraction — Extract Feeds Package
...
| Feeds package would depend on `@calendar-feeds/feed-types` + `@calendar-feeds/ics-utils` | Single dependency: `@calendar-feeds/shared` |
...
**Option A (recommended): Consumer provides plugins.** The feeds package exports utility functions that accept a plugin list. The calendar app imports plugins from `feeds/*/feed.plugin.ts` and passes them in. The aggregator has zero knowledge of specific feeds.

```typescript
// Calendar app's index.ts
import { createFeedRegistry } from "@calendar-feeds/feeds";
...
**Option B: Feeds package imports plugins directly.** Simpler for the calendar app but creates a hard dependency — any consumer gets all plugins whether they want them or not.

Option A is better. It keeps the package generic and lets consumers pick which feeds to register. The `createFeedRegistry()` factory replaces the current top-level Map construction in feed-loader.ts.

## Plan

### Phase 1: Create `packages/feeds/`

Re-introduce `packages/` for this purpose. The prior deletion of `packages/` was warranted — it held 3 tiny utility packages (265 lines total) that belonged together. The feeds package is a different beast: a substantial, purpose-built package with clear boundaries.

**Naming note:** `packages/feeds/` shares a name with the top-level `feeds/` directory (which holds feed workers). The `packages/` prefix disambiguates: `feeds/*` = deployed workers, `packages/feeds/` = library. The workspace config makes this explicit.

**Files to move from `apps/calendar/src/`:**

| Source | Destination | Notes |
|---|---|---|
| `feed-loader.ts` | `packages/feeds/src/registry.ts` | Rename to reflect its role as a registry factory |
| `feed-fetcher.ts` | `packages/feeds/src/fetcher.ts` | Unchanged logic |
| `parse-ics.ts` | `packages/feeds/src/parse-ics.ts` | Unchanged |
| `include.ts` | `packages/feeds/src/include.ts` | Unchanged |

**New files:**

```
packages/feeds/
├── package.json            ← @calendar-feeds/feeds
├── tsconfig.json
└── src/
...
```typescript
// packages/feeds/src/registry.ts
...
4. **Fetches events** via `fetchFeedEvents()` + `parseIncludeParam()` from feeds package
5. **Dispatches to renderers** (rendering — stays in calendar app)

The `fetchExternalFeed()` function currently in index.ts also moves to the feeds package — it's feed-fetching logic that uses `parseICS`.
...
```toml
[alias]
"@calendar-feeds/feeds" = "../../packages/feeds/src/index.ts"
```
...
"dependencies": {
  "@calendar-feeds/shared": "workspace:*",
  "@calendar-feeds/feeds": "workspace:*"
}
```
...
| File | Change |
|---|---|
| `packages/feeds/package.json` | Create — `@calendar-feeds/feeds` |
| `packages/feeds/tsconfig.json` | Create — extends base config |
| `packages/feeds/src/index.ts` | Create — barrel re-exports |
| `packages/feeds/src/registry.ts` | Move from `apps/calendar/src/feed-loader.ts`, refactor to factory |
| `packages/feeds/src/fetcher.ts` | Move from `apps/calendar/src/feed-fetcher.ts` |
| `packages/feeds/src/parse-ics.ts` | Move from `apps/calendar/src/parse-ics.ts` |
| `packages/feeds/src/include.ts` | Move from `apps/calendar/src/include.ts` |
| `apps/calendar/src/index.ts` | Update imports, create registry at startup, move `fetchExternalFeed` out |
| `apps/calendar/package.json` | Add `@calendar-feeds/feeds` dependency |
| `apps/calendar/wrangler.toml` | Add `[alias]` for feeds |
| `pnpm-workspace.yaml` | Add `"packages/*"` |
...
| `apps/calendar/src/feed-loader.ts` | Delete (moved to feeds package) |
| `apps/calendar/src/feed-fetcher.ts` | Delete (moved to feeds package) |
| `apps/calendar/src/parse-ics.ts` | Delete (moved to feeds package) |
| `apps/calendar/src/include.ts` | Delete (moved to feeds package) |

## What this unlocks

- **New feed consumers** only need `@calendar-feeds/feeds` + `@calendar-feeds/shared` — no rendering code
- **Calendar app identity** is explicit: a print/rendering app that uses the feeds package
- **Future renderers** (photo calendars, agenda views, print-on-demand) are clearly separate apps importing the same feeds package
- **Feed management UI or API** can import feeds without any calendar rendering baggage
- **The package is feed-agnostic** — consumers choose which plugins to register
...
5. Verify: calendar app imports from `@calendar-feeds/feeds`, not local files
